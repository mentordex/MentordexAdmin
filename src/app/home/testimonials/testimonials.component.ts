...
export class TestimonialsComponent implements OnInit {
  title: string = 'Reviews & Testimonials';
  breadcrumbs: any[] = [{ page: 'Home', link: '/home' },{page: 'Reviews & Testimonials', link: ''}]
      ...
      init: function () {      
        ...
        this.on("success", function (file, response) {
          ...
          componentObj.ngZone.run(() => {
              ...
              componentObj.addEditForm.patchValue({image_object:{ file_path: response.file, file_name: response.fileName, file_key: response.fileKey, file_mimetype: response.fileMimeType, file_category: 'team' }})
          });
  ...
  statusChange(event, id){
    ...
    this.utilsService.processPostRequest('/review/changeStatus',{is_active:event.target.checked,id:id}).pipe(takeUntil(this.destroy$)).subscribe((response) => {
       
      this.utilsService.onSuccess(environment.MESSAGES['REVIEW-SUCCESSFULLY-UPDATED']); 
  ...
  editAction(record){
    ...
    this.addEditForm.patchValue({ review: record.review})
    this.addEditForm.patchValue({ rating: record.rating})
  ...
  delete(record){
    ...
    Swal.fire({
      ...
      if (result.value) {
        this.utilsService.showPageLoader(environment['MESSAGES']['SAVING-INFO']);//show page loader
        this.utilsService.processPostRequest('/review/deleteReview',{id:record._id}).pipe(takeUntil(this.destroy$)).subscribe((response) => {
          if(response){
  ...
  onSubmit(){
    ...
    this.utilsService.processPostRequest('/review/add',this.addEditForm.value).pipe(takeUntil(this.destroy$)).subscribe((response) => {
      if(this.addEditForm.get('id').value)        
        ...
        this.utilsService.onSuccess(environment.MESSAGES['REVIEW-SUCCESSFULLY-SAVED']);
      this.cancelEdit()
      this.fetchListing()
    })
  }
  ...
  fetchListing(){
    ...
    this.utilsService.processPostRequest('/review/listing',this.pagination).pipe(takeUntil(this.destroy$)).subscribe((response) => {
